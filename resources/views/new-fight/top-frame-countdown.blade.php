...
<script>
	function timer() {
      ...
      // Countdown complete: remove the timer from the DOM
...
function challenge(icon) {
  fetch('/new-fight/challenge/{{ $creditClick->key }}/'+icon)
	...
	function redeem() {
		fetch('/new-fight/earn/redeem/{{ $creditClick->key }}')
		.then(response => {
			if (!response.ok) throw new Error(response.statusText);
    return response.text();  // parse the body as plain text
  })
...
<!-- <body bgcolor="#1F2937" onload="{{ $setTimer ?? '' }}"> -->
	<body bgcolor="#1F2937" onload="timer();">
					...
					<div>
						...
						<div class="flex justify-between gap-1">	
							...
							<h1 class="mt-2  text-4xl font-medium text-white">
								Thank You For  Voting! <br> Click The Matching Icon
							</h1>


							<div><img src="{{ $challengeIconImage }}" width="84" height="84"></div>
							<div onclick='challenge(1)' style="cursor: pointer;"><img src="/img/challenge/boxing_gloves.png" width="84" height="84"></div>
							<div onclick='challenge(2)' style="cursor: pointer;"><img src="/img/challenge/trophy.png" width="84" height="84"></div>
							<div onclick='challenge(3)' style="cursor: pointer;"><img src="/img/challenge/boxing_ring.png" width="84" height="84"></div>
							<div onclick='challenge(4)' style="cursor: pointer;"><img src="/img/challenge/explosion.png" width="84" height="84"></div>
						</div>

						<div class="flex justify-around mt-5">

							<a href='/members/profile/u/{{ $sender->username }}'>
								<img src="{{ $sender->profile_photo_url }}" class="rounded-full size-20 object-cover"/> 
								<span class="text-white">Ad By {{ $sender->name }}</span>
							</a>
							<div class="flex-col ">
								<div id="challenge_message" class="text-white">{{ $challenge_message ?? ''}}</div>
								<div id="message" class="text-white">{{ $message ?? ''}}</div>


								<!-- START TIMER -->
								<div id="timer-container" class="flex flex-col items-center space-y-4">
									<div id="display" class="text-5xl font-extrabold text-red-600"></div>
								...
								<!-- <p id="total_credits"  class="text-white">Total Credits: {{ $credits }}</p> -->
							</div>
							<p align="center" id="demo" class="text-white"></p>
						</div>
