...
<div class="w-full md:w-1/3 relative">
  <form
    id="fight-toggle-form"
    action="{{ Auth::user()->currentTeam->status === 'live' ? '/fight/stop/' : '/fight/start/' }}"
    method="POST"
  ...
  >
    ...
    >
      <span id="fight-toggle-icon" class="text-xl transform transition-transform duration-300 group-hover:animate-bounce">
        🥊
      </span>
      <span id="fight-toggle-label">
        {{ Auth::user()->currentTeam->status === 'live' ? __('Fight Is Live!') : __('Start Fight') }}
      </span>
    </button>
  </form>
...
<script>
  (function() {
    const form  = document.getElementById('fight-toggle-form');
    const btn   = document.getElementById('fight-toggle-btn');
    const icon  = document.getElementById('fight-toggle-icon');
    const label = document.getElementById('fight-toggle-label');
    ...
    form.addEventListener('submit', function(e) {
      ...
      .then(json => {
        // Toggle action URL
        const isNowLive = form.action.includes('/fight/start/');
        form.action = isNowLive ? '/fight/stop/' : '/fight/start/';

        // Update button text
        label.textContent = isNowLive ? 'Fight Is Live!' : 'Start Fight';

        // Optionally swap colors: red ↔ green
        btn.classList.toggle('bg-red-600', !isNowLive);
        btn.classList.toggle('hover:bg-red-700', !isNowLive);
        btn.classList.toggle('bg-green-600', isNowLive);
        btn.classList.toggle('hover:bg-green-700', isNowLive);

        // Optionally change icon
        icon.textContent = isNowLive ? '🔥' : '🥊';
      })
